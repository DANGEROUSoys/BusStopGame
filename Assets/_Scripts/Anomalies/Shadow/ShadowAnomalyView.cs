...
public class ShadowAnomalyView : MonoBehaviour
{
    ...
    public void Initialize(float timeBeforeScreamer)
    {
        ...
        _face.transform.localScale = new Vector3(_minScale, _minScale, _minScale);
    }

    public void ChangeShadowScale(float currentTime)
    {
        float currentScale = currentTime * _scaleCoefficient + _minScale;
        _face.transform.localScale = new Vector3(currentScale, currentScale, currentScale);;
    }

    public void ShowAppearingAnimation()
    {
        _shadowVisual.SetActive(true);
    }
}
